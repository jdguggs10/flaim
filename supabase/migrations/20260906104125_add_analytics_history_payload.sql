-- FLA-265: add a parity sibling for the analytics dashboard payload that reads
-- permanent ET-day user history through the close marker and raw events after
-- it. The existing dashboard_payload() and both snapshot refresh overloads are
-- deliberately untouched; switching readers is a later promotion gate.
--
-- The body of 20260802131749_add_sync_recent_dashboard_payload.sql and the
-- current hosted dashboard payload were rechecked on 2026-09-06. Their raw
-- source hashes differ because migration comments are not retained in hosted
-- prosrc, but the normalized executable SQL was identical. This sibling starts
-- from that complete body and changes only the history-backed metrics plus the
-- explicitly bounded 30-day health window.
--
-- Hosted application, parity acceptance, and any dashboard switch remain
