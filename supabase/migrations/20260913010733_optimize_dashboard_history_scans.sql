-- FLA-378: remove the two production-proven hot paths from the history-backed
-- dashboard payload without changing its JSON contract, sources, windows,
-- grants, cadence, or retention policy.
--
-- 1. The open raw bridge compared an America/New_York date expression on every
--    event with the close marker. That expression could not use the existing
--    `mcp_tool_events(ts)` index, so every five-minute refresh scanned and
--    sorted the entire retained raw table merely to keep rows after the marker.
--    Comparing `ts` with the UTC instant for the next ET midnight is exactly
--    equivalent and sargable, including across daylight-saving transitions.
--
-- 2. `user_concentration` selected each user's call-weighted client mode with
--    a correlated subquery over `history_calls`. Production had 5,776 users,
--    so one payload reread the complete materialized history 5,776 times. The
--    replacement groups client totals once, applies the same call-count and
--    lexical tie-break with `distinct on`, and joins the result to per-user
--    totals. NULL client names remain ignored exactly as before.
--
-- Production EXPLAIN evidence before this migration:
-- - complete payload: 52.0 s, 1,588,097 temporary blocks read;
-- - user concentration alone: 36.8 s, 1,564,218 temporary blocks read;
-- - equivalent set-based client mode: 1.1 s, 4,965 temporary blocks read.
-- A read-only inline run of the complete optimized payload took 7.3 s against
-- the same production data and compared byte-identical to the deployed JSON.
--
-- Full CREATE OR REPLACE of the current FLA-361 body. Hosted application and
-- every snapshot refresh remain separate approval gates.
