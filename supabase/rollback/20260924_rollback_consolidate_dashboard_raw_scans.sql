...
-- 20260924191500_consolidate_dashboard_raw_scans.sql (FLA-412).
--
...
--
-- It restores analytics.dashboard_payload_history(boolean) to the reviewed
-- FLA-378 body with the FLA-388 60-day stale guard: the `recent_ev` rolling
-- CTE and four separate raw health scans. FLA-412 changed performance only,
-- so rolling back changes query cost, not the payload.
--
-- The restored body below is reproduced VERBATIM from
