...
const AnnotationPill = ({ annotation }: { annotation: Annotation }) => {
  const className =
    "inline-block text-nowrap px-3 py-1 rounded-full text-xs max-w-48 shrink-0 text-ellipsis overflow-hidden bg-secondary text-muted-foreground";
  ...
  switch (annotation.type) {
    ...
    case "container_file_citation":
      return (
        <a
          href={`/api/chat/container_files/content?file_id=${annotation.fileId}${annotation.containerId ? `&container_id=${annotation.containerId}` : ""}${annotation.filename ? `&filename=${encodeURIComponent(annotation.filename)}` : ""}`}
          download
...
const Annotations = ({ annotations }: { annotations: Annotation[] }) => {
  ...
  return (
    <div className="flex max-w-full mr-28 ml-4 overflow-x-auto gap-2 mb-2">
