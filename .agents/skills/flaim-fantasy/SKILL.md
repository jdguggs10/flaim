...
Ten tools read league data. `refresh_leagues` is the only bounded write tool, and it changes only Flaim's connected-league records and discovery metadata. Most league data tools require `platform`, `sport`, `league_id`, and `season_year`. The main exceptions are `get_user_session` (no parameters), `refresh_leagues` (optional `platforms`), `get_ancient_history` (optional `platform` only), and `get_draft` (optional `season_year`, plus Sleeper-only `draft_id`).
