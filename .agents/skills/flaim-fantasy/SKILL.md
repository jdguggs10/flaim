...
7. Never infer league ownership from `market_percent_owned`, `percentOwned`, or `ownership_scope`. For a targeted "who owns X?" or "is X available?" question, use `get_players` for the selected league. Sleeper's `availability_status` is authoritative because the matched player ID is checked against every current roster for that exact league. If league ownership fields are absent or null, enumerate teams via `get_league_info` and use `get_roster`.
...
- "Who owns Player X in my league?" → `get_user_session` → `get_league_info` → `get_players`; use its league fields, never market ownership, and fall back to `get_roster` per team only if league ownership is absent or null
