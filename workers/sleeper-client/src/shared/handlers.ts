...
export function createSleeperHandlers(config: SleeperSportConfig): Record<string, HandlerFn> {
  const { sport, statePath } = config;
  const handleGetFreeAgents = createSleeperGetFreeAgentsHandler(sport);
  ...
  async function handleSearchPlayers(
    ...
    try {
      ...
      const playersIndex = await getSleeperPlayersIndex(env, sport);
  ...
  async function handleGetMatchups(
    ...
    try {
      ...
      if (!matchupWeek) {
        const stateRes = await sleeperFetch(statePath);
        if (stateRes.ok) {
  ...
  async function handleGetTransactions(
    ...
    try {
      const currentWeek = week || await getSleeperCurrentWeek(statePath);
