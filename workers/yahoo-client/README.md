...
- `is_keeper` is present on **every** player once a league is keeper-format, with `status`/`kept` explicitly `false` for non-kept players — not merely absent. `isKeeper` is only omitted when the field itself is absent from Yahoo's response (i.e. the league isn't keeper-format, or Yahoo didn't return it for this resource); when the field is present but its `status`/`kept` sub-values use an unrecognized encoding, `isKeeper` is still emitted with those keys individually `undefined`.
