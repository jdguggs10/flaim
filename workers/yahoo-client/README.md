...
Each player's `points` field is emitted only when Yahoo's own echoed `player_points.coverage_type` is `week` with a finite `week` value — a season-to-date total Yahoo might echo on a `current` request is never mislabeled as this week's score. A genuine `0.00` total (e.g. a bye-week starter) is preserved as `0`; a player with no usable stats sub-resource simply omits `points` (never `null`). Whenever at least one player carries usable points, the response adds `pointsCoverage: { type: 'week', week: N }`; whenever no player does, it adds `limitations.playerPointsAvailable: false` instead.
