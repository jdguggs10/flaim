...
`percent_owned` arrives as an array of single-key objects (`[{coverage_type}, {week|date}, {value}, {delta}]`), so `value` is found by key at any position; the flattened-object and numeric-keyed container forms are accepted too. Only `value` is read, so football's `week` coverage and the daily sports' `date` coverage need no separate handling. A genuine `0` rate is preserved; a missing sub-resource or non-numeric value yields `null`.
