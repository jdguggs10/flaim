...
**Football only** — Yahoo's `;week=` roster selector is itself football-only, and category/roto leagues carry no `player_points` resource at all even when the sport supports it, so this stays scoped to the one case Yahoo's docs actually describe. On a football roster request, the handler always appends `/players/stats;type=week` to the roster fetch (plus `;week=N` when the snapshot pins a specific week; a `current` request sends no explicit week and lets Yahoo default to whatever it currently scores).

Sub-resources on a Yahoo player object (`selected_position`, `player_points`, etc.) land as extra indexed entries after the index-0 metadata array, but Yahoo doesn't document or guarantee their order — `findPlayerSubResource` (`shared/handlers/utils.ts`) scans by key instead of assuming a fixed index, and both `selected_position` and `player_points` now go through it.

Each player's `points` field is gated on Yahoo's own echoed `player_points` coverage, and the exact rule differs by snapshot type. On a `week` snapshot, a player's points are usable only when Yahoo echoes `coverage_type: 'week'` AND the echoed `week` EXACTLY equals the requested week — any other week (Yahoo drifting to an adjacent week, say) is treated as unusable for that player. On a `current` snapshot there is no requested week to check against, so instead every emitted player's points must share one single consistent week: the coverage week of the first player with usable (finite points, `coverage_type: 'week'`, positive-integer week) data is taken as that response's week, and any other player whose echoed week differs is treated as unusable — so a season-to-date total Yahoo might echo, or players scored against different weeks, can never be mislabeled or mixed together as "this week's score". A genuine `0.00` total (e.g. a bye-week starter) is preserved as `0`; a player with no usable points under this gate simply omits `points` (never `null`). Whenever at least one player ends up with usable points, the response adds `pointsCoverage: { type: 'week', week: N }`; whenever no player does, it adds `limitations.playerPointsAvailable: false` instead.

**Status: fixture-tested only, not live-verified.** There is no captured `player_points` payload anywhere in this repo — the request shape and sub-resource index are inferred from Yahoo's own (sparse) documentation. Yahoo cannot be exercised on the preview lane, so the first live confirmation is a production check after merge; if that check shows no `points`, the parser degrades to today's behavior (field omitted, `playerPointsAvailable: false`).
