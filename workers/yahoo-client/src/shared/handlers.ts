...
import { asArray, getPath, unwrapLeague, unwrapTeam, logStructure, parseYahooPercentOwned } from './normalizers';
...
export function createYahooHandlers(config: YahooSportConfig): Record<string, HandlerFn> {
  const { sport, getPositionFilter, extraLeagueFields } = config;
  ...
  async function handleGetLeagueInfo(
    ...
    try {
      ...
      const raw = await response.json();
      logStructure(`get_league_info raw (${sport})`, raw);
  ...
  async function handleGetStandings(
    ...
    try {
      ...
      const raw = await response.json();
      logStructure(`get_standings raw (${sport})`, raw);
  ...
  async function handleGetRoster(
    ...
    try {
      ...
      const raw = await response.json();
      logStructure(`get_roster raw (${sport})`, raw);
  ...
  async function handleGetMatchups(
    ...
    try {
      ...
      const raw = await response.json();
      logStructure(`get_matchups raw (${sport})`, raw);
  ...
  async function handleGetFreeAgents(
    ...
    try {
      ...
      const raw = await response.json();
      logStructure(`get_free_agents raw (${sport})`, raw);
