  ...
  try {
    ...
    const response = await espnFetch(path, GAME_ID, {
      ...
      league: {
        ...
        historical: canonicalYear < getCurrentSeasonYear('hockey'),
      },
    });
  ...
  try {
    ...
    const bracketFinal = seasonComplete && !hasExplicitFinalRanks(teams)
      ? await fetchBracketFinal(
        ...
        canonicalYear < getCurrentSeasonYear('hockey'),
      )
      : null;
...
async function handleGetRoster(
  ...
  if (snapshot.type === 'week') {
    return rosterSnapshotUnsupportedError('espn', 'hockey');
  }
  ...
  try {
    ...
    let path = `/seasons/${espnYear}/segments/0/leagues/${league_id}?view=mRoster&view=mTeam&view=mSettings`;
...
async function handleGetFreeAgents(
  ...
  try {
    ...
    const freeAgents = players.map((entry) => {
      ...
      // Category and rotisserie hockey leagues get no applied totals from
...
async function handleGetTransactions(
  ...
  try {
    assertTransactionsSeasonSupported('hockey', canonicalYear);
