...
export function getDefaultPositionName(positionId: number): string {
  ...
  if (!UNKNOWN_DEFAULT_POSITION_IDS.has(positionId)) {
    ...
    console.warn(`[baseball-mappings] Unknown defaultPositionId: ${positionId}`);
  }
  return `POS_${positionId}`;
}

/**
 * Get lineup slot name from ESPN lineupSlotId or eligibleSlots field
 * Use this for ROSTER SLOTS (where a player can be placed in lineup)
 */
export function getLineupSlotName(slotId: number): string {
  ...
  if (!UNKNOWN_LINEUP_SLOT_IDS.has(slotId)) {
    ...
    console.warn(`[baseball-mappings] Unknown lineupSlotId: ${slotId}`);
  }
  return `SLOT_${slotId}`;
}
 ...
 * Note: eligibleSlots uses LINEUP_SLOT_MAP IDs (roster slots), not DEFAULT_POSITION_MAP
 * Filters out Bench/IL slots for cleaner output
 */
export function transformEligiblePositions(slots: number[]): string[] {
  return slots
    .map(slot => LINEUP_SLOT_MAP[slot])
    .filter((name): name is string => !!name && !['Bench', 'IL'].includes(name));
}
