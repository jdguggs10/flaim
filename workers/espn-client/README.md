...
`get_roster` player entries also omit `proTeam` and `injuryStatus` on any historical snapshot (`week` or `date`), adding `playerProTeamAvailable: false` to the `limitations` block (FLA-278). ESPN's `mRoster` payload exposes `proTeamId`/`injuryStatus` on the top-level player object as CURRENT-club and CURRENT-injury state only — there is no historical value to return for a past week/date, so a player traded or whose status changed since that snapshot would otherwise show present-day facts mislabeled as historical. Current-roster entries are unaffected; both fields are returned exactly as before.
