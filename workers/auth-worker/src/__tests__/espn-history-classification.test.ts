...
describe('classifyEspnHistoryUpstreamError', () => {
  ...
  it.each([new EspnApiError('rate', 429), new EspnApiError('server', 500), new EspnApiError('unknown'), new Error('network'), new DOMException('timeout', 'TimeoutError')])('classifies retryable failures', (error) => expect(classifyEspnHistoryUpstreamError(error)).toBe('retryable'));
});
