...
For a bounded incident capture, `POST /internal/support/yahoo/capture-game-raw` accepts a numeric game key and an optional `collection` value of `leagues` (the default) or `teams`. Each value maps to one fixed Yahoo user-game URL; no provider path, query, or response selector can be supplied. The route keeps its two-secret gate, rate limit, token guards, raw-byte cap, no-store response, and existing capture marker.
