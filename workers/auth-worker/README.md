...
Discovery uses the unfiltered `game_types=full` collection as its primary all-sports, all-history request. When that valid response lacks current-season football, persisted discovery and the read-only reconciliation path make exactly one narrower current-NFL request, then merge by Yahoo league key while preserving the broad record on duplicates. The supplement is best-effort: its failure emits the bounded `yahoo_current_football_fallback_failed` warning and never discards usable broad results. Yahoo's own semantics for `game_types=full` cover every standard draft-and-trade fantasy game, not only `nfl`/`mlb`/`nba`/`nhl`: Yahoo Fantasy College Football (`cfb`) is one such game, and is a confirmed source of unsupported-sport-code drops in production (FLA-365). Yahoo also historically ran separate "Plus" game codes (`pnfl`, `pmlb`, and by the same pattern presumably `pnba`/`pnhl`) for football and baseball before folding them into the free game starting with the 2010 season, so an account old enough to predate that merge could in principle carry a supported-sport league under a code `SPORT_CODE_MAP` doesn't recognize — unconfirmed against Flaim's own data as of 2026-09. `SPORT_CODE_MAP` (`yahoo-connect-handlers.ts`) intentionally maps only `nfl`/`mlb`/`nba`/`nhl`; any other code is dropped per-game during parsing, not account-wide, so a dropped game's leagues never block a sibling game's supported-sport leagues from syncing in the same discovery response. Every drop is now visible without a support-tool run: `logYahooDiscoveryDropIfAny` emits a structured `yahoo_discovery_drop` warning (findable in Cloudflare Logs) carrying `unsupported_sport_codes`, so a future unmapped code is discoverable operationally rather than requiring this paragraph to be an exhaustive registry.
