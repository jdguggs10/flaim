...
Yahoo authorization-code exchange and refresh-token exchange intentionally use different token request bodies:
- Authorization-code exchange sends `redirect_uri`. This must continue to match the registered Yahoo callback.
- Refresh-token exchange omits `redirect_uri` by default. `YAHOO_REFRESH_GRANT_REDIRECT_URI=include` exists only as a compatibility escape hatch.

Do not copy the authorization-code token body into the refresh-token grant. A May 2026 production incident showed recurring `YAHOO_AUTH_UNAVAILABLE` / `refresh_temporarily_unavailable` failures after the 1-hour Yahoo access token expired while the stored refresh token was still valid. Live logs showed the old refresh request failing with `request_has_redirect_uri=true`; after deploying `request_has_redirect_uri=false`, Yahoo accepted the refresh, returned a fresh 1-hour access token, and `get_league_info`, `get_roster`, `get_standings`, and `get_matchups` all succeeded. Yahoo examples may include `redirect_uri` on refresh, but Flaim's live app flow is currently validated only without it.
...
Cooldown is a circuit breaker for retryable Yahoo token-refresh failures, not a generic error blanket. It should apply to confirmed transient/backoff classes such as Yahoo `429`/`999`, upstream `5xx`, fetch/timeout failures, and Yahoo-provided `Retry-After`. It should not apply to permanent or malformed OAuth failures such as `invalid_grant`, revoked/expired refresh tokens, `invalid_request`, `invalid_client`, `unsupported_grant_type`, redirect URI mismatches, missing local Yahoo client credentials, or ambiguous text-only `4xx` responses without upstream backoff. Those release the lease and surface the original failure so reconnect/config/debugging can happen immediately. `YAHOO_REFRESH_COOLDOWN_MODE=disabled` remains available as an emergency diagnostic bypass, but production and preview should not normally set it.

Refresh diagnostics are emitted as structured, non-secret `yahoo-connect` log events with correlation IDs. The refresh path classifies failures with `diagnostic_class` values such as `yahoo_rate_limit`, `yahoo_transient_http`, `yahoo_transient_text`, `fetch_error`, `timeout`, `lease_wait_timeout`, and `lease_budget_exhausted`, plus `outcome`, retry delay, request-timeout, and lease-budget fields where relevant. Token-endpoint diagnostics also include the grant type, callback URL/host/path, whether `redirect_uri` was sent, whether Yahoo client credentials were configured, the caller auth type, response body class, and `retry_after_source` (`upstream_header` or `fallback_default`) so incidents can distinguish Yahoo-provided backoff from Flaim defaults. These fields are intended for production incident triage without exposing access tokens, refresh tokens, rosters, or raw Yahoo API payloads.
