...
The health check performs a bounded, payload-free `HEAD` request against a core
table through the Supabase Data API. An empty result is healthy; missing
configuration or any PostgREST/transport error returns `503` with a sanitized
`supabase_status`. Roll back a bad preview deployment with `corepack pnpm --dir
workers/auth-worker exec wrangler rollback --env preview`, then verify `/health`
again before considering production.
...
Yahoo rate-limit-like token refresh failures (`429`/`999`) convert the active lease into a short shared `cooldown:<owner>` marker. Yahoo's `Retry-After` header wins when present; otherwise token refresh uses a 60-second fallback cooldown because Yahoo token-refresh 429 responses often omit `Retry-After`. Permanent, malformed, local config, fetch, and timeout failures do not get hidden behind that cooldown path; they release or retain the short lease according to the failure class and preserve upstream diagnostics for reconnect/debugging.
...
Do not broaden the Yahoo cooldown path without proving Yahoo is asking Flaim to back off and preserving the original upstream error. The earlier broad cooldown path masked malformed/permanent refresh failures as a generic temporary outage. Current behavior limits persisted cooldowns to rate-limit-like token refresh responses, while permanent failures surface the upstream status in responses/logs so reconnect/config/debugging can happen immediately. Retryable failures still return `refresh_temporarily_unavailable`, `retryable: true`, and `Retry-After` metadata so clients can back off without prompting reconnect.

League discovery (`POST /connect/yahoo/discover`, and `refresh_leagues` through `/internal/leagues/refresh`) treats a Yahoo 403 whose body says the *application* is not authorized as Yahoo's platform-wide app-approval denial, not a user problem: the response stays `yahoo_api_error` / 502 but `error_description` carries the shared app-review outage message from `@flaim/worker-shared` (`YAHOO_APP_REVIEW_OUTAGE_MESSAGE.discovery`, the same words yahoo-client uses on the data path, plus "the Yahoo link is saved; nothing needs to be redone"). It is deliberately not `retryable`: the web routes retryable discovery failures into the token-cooldown notice, which hides `error_description` and ends "reconnect Yahoo" — the one instruction that cannot help — while the non-retryable path shows the description verbatim. The discovery wording also avoids the words the fantasy-mcp user-session widget's text heuristic reads as reconnect-required (`connect`, `auth`, `credential`, `expired`, `revoked`) or retry-later (`try again`, `rate limit`); a fantasy-mcp test pins that against the real classifier. A resource-level 403 keeps the generic `Yahoo API returned 403`.

When Yahoo's user-scoped discovery collection is demonstrably incomplete for one account, the operator-only `POST /internal/support/yahoo/recover-league` route can recover one current league by full Yahoo `league_key`. It requires the ordinary internal-service secret plus the separate support-tool secret, validates direct Yahoo metadata (exact key, supported sport, current numeric season, nonblank name), proves exactly one owned team from the active OAuth identity, follows the renew chain with a bounded fail-closed walk, and performs one idempotent stored-league upsert that requires the resolved root column to be present. The entire provider walk has a 40-second deadline and checks it before the write. It never calls discovery, changes `provider_sync_state`, or accepts league/team metadata from the operator. Its response contains only masked-account/correlation timing fields and one closed persistence status; it exposes no provider identifiers or names.

For a bounded incident capture, `POST /internal/support/yahoo/capture-game-raw` keeps the original numeric game-key request (`collection` is `leagues` by default or `teams`) and also accepts two explicit fixed targets: `discovery` captures the exact production all-history discovery resource, and `league-teams` accepts one full numeric Yahoo `leagueKey` for its direct teams resource. The request contract is closed: each target rejects fields for the others, and no provider path, query, URL, or response selector can be supplied. Every target keeps the two-secret gate, rate limit, guarded token path, one-fetch manual-redirect behavior, raw-byte cap, token-content refusal, no-store response, and existing capture marker.

Discovery uses the unfiltered `game_types=full` collection as its primary all-sports, all-history request. When that valid response lacks current-season football, persisted discovery and the read-only reconciliation path make exactly one narrower current-NFL request, then merge by Yahoo league key while preserving the broad record on duplicates. The supplement is best-effort and has a 10-second timeout: its failure emits the bounded `yahoo_current_football_fallback_failed` warning, never discards usable broad results, and leaves the refresh successful but potentially incomplete. Yahoo's own semantics for `game_types=full` cover every standard draft-and-trade fantasy game, not only `nfl`/`mlb`/`nba`/`nhl`: Yahoo Fantasy College Football (`cfb`) is one such game, and is a confirmed source of unsupported-sport-code drops in production (FLA-365). Yahoo also historically ran separate "Plus" game codes (`pnfl`, `pmlb`, and by the same pattern presumably `pnba`/`pnhl`) for football and baseball before folding them into the free game starting with the 2010 season, so an account old enough to predate that merge could in principle carry a supported-sport league under a code `SPORT_CODE_MAP` doesn't recognize — unconfirmed against Flaim's own data as of 2026-09. `SPORT_CODE_MAP` (`yahoo-connect-handlers.ts`) intentionally maps only `nfl`/`mlb`/`nba`/`nhl`; any other code is dropped per-game during parsing, not account-wide, so a dropped game's leagues never block a sibling game's supported-sport leagues from syncing in the same discovery response. Every drop is now visible without a support-tool run: `logYahooDiscoveryDropIfAny` emits a structured `yahoo_discovery_drop` warning (findable in Cloudflare Logs) carrying `unsupported_sport_codes`, so a future unmapped code is discoverable operationally rather than requiring this paragraph to be an exhaustive registry.

Refresh diagnostics are emitted as structured, non-secret `yahoo-connect` log events with correlation IDs. The refresh path classifies failures with `diagnostic_class` values such as `yahoo_rate_limit`, `yahoo_transient_http`, `yahoo_transient_text`, `yahoo_permanent`, `fetch_error`, `timeout`, `cooldown_active`, `lease_not_acquired`, and `lease_window_too_short`, plus `outcome`, request-timeout, and lease fields where relevant. Token-endpoint diagnostics also include the grant type, callback URL/host/path, whether `redirect_uri` was sent, whether Yahoo client credentials were configured, the caller auth type, response body class, sanitized upstream body text, `retry_after_source` (`upstream_header`, `fallback_default`, or `cooldown_remaining`), and non-secret refresh-token rotation booleans (`refresh_token_returned`, `refresh_token_changed`, recovery fields) so incidents can distinguish Yahoo-provided backoff from Flaim defaults, active cooldown waits, and storage races. These fields are intended for production incident triage without exposing access tokens, refresh tokens, rosters, or raw Yahoo API payloads.
