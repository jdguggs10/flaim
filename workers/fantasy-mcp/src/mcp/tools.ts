...
const standingsEntrySchema = looseObject({
  ...
  waiverPriority: z
    ...
    .nullable()
    .optional()
    .describe("Yahoo and Sleeper: the team's current waiver-claim priority, 1 = first. Present for rolling-priority leagues and for FAAB leagues that use priority as their bid tie-breaker. Null when priority does not apply. Absent (not null) on ESPN. Distinct from get_transactions' waiver_priority, which is a past claim's priority."),
  faabBalance: z
    .number()
    .nullable()
    .optional()
    .describe('Yahoo and Sleeper: remaining FAAB (free-agent budget) balance, after FAAB traded between teams. Null for leagues that do not use FAAB. Absent (not null) on ESPN.'),
});
