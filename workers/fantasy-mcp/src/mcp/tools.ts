...
export function getUnifiedTools(): UnifiedTool[] {
  ...
  return [
    ...
    // -------------------------------------------------------------------------
    {
      ...
      description: `Get season standings and outcome snapshot; includes verified season-outcome fields when available. Returns team records, rankings, and points summaries. Also returns seasonPhase (regular_season/playoffs_in_progress/season_complete), seasonComplete, and per-team outcome fields: finalRank, championshipWon, playoffOutcome, outcomeConfidence, madePlayoffs, playoffSeed. Outcome fields are null when not verifiable — do not infer championship from rank or team name. ESPN may also include projected-rank fields. Best used after get_user_session and, in most league-specific chats, after get_league_info so team names and league context are already established. For historical finish questions, call get_ancient_history first to discover seasons, then call this tool per season for verified outcomes. Read-only and safe to retry. Current date is ${currentDate}.`,
      inputSchema: {
        platform: z
          .enum(['espn', 'yahoo', 'sleeper'])
          .describe('Fantasy platform (e.g., "espn", "yahoo", "sleeper")'),
