...
const standingsEntrySchema = looseObject({
  ...
  waiverPriority: z
    ...
    .nullable()
    .optional()
    .describe("Yahoo only: the team's current waiver-claim priority, 1 = first. Present for rolling-priority leagues and for FAAB leagues that use priority as their bid tie-breaker. Null when priority does not apply. Absent (not null) on ESPN and Sleeper. Distinct from get_transactions' waiver_priority, which is a past claim's priority."),
  faabBalance: z
    .number()
    .nullable()
    .optional()
    .describe('Yahoo only: remaining FAAB (free-agent budget) balance. Null for leagues that do not use FAAB. Absent (not null) on ESPN and Sleeper.'),
});
