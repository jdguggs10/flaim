...
export function getUnifiedTools(): UnifiedTool[] {
  ...
  return [
    ...
    // -------------------------------------------------------------------------
    {
      ...
      description: `Search for player identity by name. Always returns identity fields, but ownership context varies by platform. ESPN and Yahoo return market/global ownership and can also populate league ownership fields when credentials and league context are available. Sleeper always evaluates league ownership against every current roster in the selected league, except during an active draft, when this tool returns an error instead of an answer, since draft picks aren't yet reflected on rosters. Sleeper adds a Sleeper-only league_team_id (the Sleeper roster id, or null); its market/global ownership stays unavailable regardless (ownership_scope="unavailable", market_percent_owned=null) — league_team_id is league ownership, not market data. For a selected active league, use established session context (call get_user_session only if needed), then get_league_info so league-specific ownership and team names can be resolved. League ownership fields: league_status ("ROSTERED" = on a team, "FREE_AGENT" = unrostered in this league, null = ownership unresolved, e.g. ambiguous rostering), league_team_name (fantasy team name if rostered), league_owner_name (team owner if rostered). FREE_AGENT is an ownership fact from a roster scan, not an acquisition guarantee — it says nothing about waiver state, so never turn it into pickup advice; get_free_agents is what reports players available to acquire. Trust a non-null league_status directly — null team/owner fields alongside FREE_AGENT are expected, not a signal to verify. Fall back to get_roster only when league_status itself is absent or null. In a Yahoo keeper league, entries may add isKeeper as status, cost, and kept; Yahoo has never been observed populating cost, so no keeper cost is available. Use values from get_user_session. Read-only. Current date is ${currentDate}.`,
      inputSchema: {
        query: z
          .string()
          .min(2)
