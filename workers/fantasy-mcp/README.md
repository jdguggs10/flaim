...
Retryable platform errors preserve `status`, `retryable`, and `retry_after` metadata in tool `structuredContent` and `_meta`; Yahoo rate limits and transient token-refresh contention should be retried after the provided delay instead of treated as a reconnect prompt.
