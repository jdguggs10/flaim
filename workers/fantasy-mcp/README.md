...
`get_transactions` uses platform-specific week semantics in v1. ESPN `week` always means matchup period, including daily sports; omitting it selects the current and previous matchup periods, and responses expose the exact provider scoring periods plus source/limitation metadata. ESPN currently returns completed activity-feed rows and rejects failed-bid or trade-lifecycle-only filters with `ESPN_TRANSACTION_TYPE_UNAVAILABLE`. Sleeper supports explicit week. Yahoo ignores explicit `week` and uses a recent 14-day timestamp window for completed league transactions; `type=waiver` and `type=pending_trade` return pending items for the authenticated user's own team. ESPN responses include a `teams` map (team ID → display name). Player entries are enriched with name, position, and pro team when time permits.
...
```typescript
{
  ...
  week?: number;                 // For matchups; transaction matchup periods on ESPN, transaction weeks on Sleeper; historical get_roster on football (all platforms) + Sleeper basketball. On ESPN/Yahoo daily sports, get_roster week is ignored (current roster + snapshot note) for published-client compatibility
