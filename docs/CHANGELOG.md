...
- **Added**: The v2 league widget (`ui://widget/user-session-v2.html`) now ends with a muted footer crediting "Fantasy data provided by Yahoo Fantasy, ESPN, and Sleeper", with Yahoo Fantasy linking to the official Yahoo Fantasy site (allowlisted in the v2 widget CSP redirect domains only); the frozen v1 widget resource is byte-unchanged, and the web app footer already carries the same attribution.
