...
- **Fixed (description audit)**: two independent audits caught follow-up inaccuracies in the text above and completed two clauses left unfinished by the earlier get_players ownership change (FLA-385/PR #290) — `get_players`' description and the server instructions now also cover the silent `SLEEPER_OWNERSHIP_UNAVAILABLE` path (league/roster/user load failure outside an active draft) alongside the existing draft-in-progress case, and state that a null `league_status` means unavailable credentials/rosters on ESPN/Yahoo versus unavailable-or-ambiguous rosters on Sleeper, rather than one Sleeper-flavored explanation for all three platforms.
