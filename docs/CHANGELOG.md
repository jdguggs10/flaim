...
- **Improved**: the read-only Yahoo membership verifier also probes Yahoo's direct logged-in-user teams collection and records only whether it parsed and whether the requested league produced zero, one, or multiple team-key entries. It accepts Yahoo team entities in either one-record object or array-of-fragments form, and numeric or constrained lowercase symbolic game/league key segments on non-target entries, while the requested target remains separately numeric and exact. It keeps distinct closed failure categories for invalid wrappers, entity shapes, no direct `team_key` field, multiple direct `team_key` fields, one non-string value, and one noncanonical string. The extra evidence stays in redacted operator telemetry; it does not change the support response, recovery selection, or persistence behavior.
