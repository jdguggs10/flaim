...
- **Added**: the platform-dependent keeper field inventory now lives on the tools that return those fields. `get_league_info` names ESPN `keeperSettings`/`isKeeperLeague` and the raw per-team `keeperPlayerIds`/`futureKeeperPlayerIds`, Sleeper `leagueFormat` with the warning that `typeRaw` is an undocumented convention that must not be read on its own, and Yahoo `draftType`/`isAuctionDraft`/`canTradeDraftPicks` with no keeper-cost rule. `get_roster` names ESPN `keeperValue`/`keeperValueFuture` and the response-level `keeperValueUnit`, the historical-snapshot withholding of `keeperValueFuture`, and that Sleeper's resolved `keepers` is non-empty only during its pre-draft keeper-selection window. `get_roster`, `get_free_agents`, and `get_players` all note that Yahoo's `isKeeper.cost` has never been observed populated.
...
- **Added**: server instructions gained two cross-tool rules — keeper cost is a league-specific house rule that no platform computes after a trade, so ask for the league's convention; and a missing league record never turns into asking the user for a numeric league ID or season value.
