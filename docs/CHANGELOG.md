...
- **Added**: Every widget resource read (`user-session-widget`, `-v2`, `-v3`) now emits a structured `widget_resource_read` log line (`uri`, `resource_name`, `correlation_id`, `client_name`) so v1's share of reads — and thus its retirement readiness — is computable from observability data instead of guessed at.
