...
- **Added**: Sleeper's `get_players` now resolves each matched player against that league's current rosters and adds four additive fields: `league_status` (`"ROSTERED"` | `"FREE_AGENT"`) and `league_team_id` / `league_team_name` / `league_owner_name` (populated when rostered, `null` when a free agent). Previously `get_players` could only return player identity — it had no way to answer "who owns Player X in my league?" or "is Player X available?". `league_id` is now required for `get_players`; a request without it returns `MISSING_PARAM`. The rosters/users fetch is not best-effort: a failed fetch fails the whole call rather than ever reporting a player as a `FREE_AGENT` on incomplete data. Sleeper's existing market-ownership fields (`market_percent_owned: null`, `ownership_scope: "unavailable"`) are untouched — league availability (this-league-only) and market ownership (cross-league popularity, which Sleeper doesn't expose) are deliberately kept separate.
