...
- **Fixed**: Nested Yahoo team metadata and manager entities serialized as a single object now count as complete membership evidence, matching the provider shape already handled by the Yahoo client. Flat team metadata and array-shaped manager entities remain supported, and every form still requires exactly one canonical nonblank GUID before a mismatch can be decisive.
