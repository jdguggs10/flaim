...
- **Changed**: Yahoo refresh-token grants now omit `redirect_uri` again after production logs showed the include-redirect shape returning Yahoo `429` / `Too many failed requests` on the first lazy refresh after reconnect.
