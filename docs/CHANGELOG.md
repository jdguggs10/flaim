...
- **Improved**: failed recovery writes log one closed internal stage category so operators can distinguish credential, provider-shape, ownership, renew-chain, deadline, readback, and persistence failures without exposing the category or any customer/provider identifiers to the support client.
