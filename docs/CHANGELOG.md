...
- **Changed**: when the preference is on, `get_user_session` adds `widget: { hidden: true }` to its `structuredContent`; the key is absent otherwise, so default responses are unchanged. The declared output schema gains that optional `widget` property. Every published league widget URI honors it: the widget renders nothing and reports a zero size, and shows again if the preference is turned off mid-chat. Leagues are still returned to the model; only the card is suppressed. The auth-worker treats a database without the column as the preference being off. Production already has the column, so the new migration is a no-op there.
...
- **Changed**: `health_summary`, `health_summary_7d`, `tool_health`, and `tool_health_7d` now come from one grouping-sets pass over the 30-day raw window, with the seven-day values computed as filtered aggregates of the same rows. The four separate scans measured about 28 seconds combined; the single-pass prototype measured 8.0 seconds. Payload keys, value types, values, rounding, windows, and ordering expressions are unchanged, as are grants, cadence, and retention; the order of tool rows tied on `calls` was unspecified before and remains unspecified. The migration and a reviewed rollback each verify the exact function body they replace by `md5(prosrc)` digest before running. The rollback that restores the previous body lives in `supabase/rollback/`.
...
- **Improved**: the bounded Yahoo support capture can now observe either its original fixed `users → game → leagues;out=teams` collection (the default) or the separately fixed `users → game → teams` collection. The new `collection` field is a closed two-value enum, so operators still cannot supply a Yahoo path, query, or response selector; both choices retain the two-secret gate, rate limit, guarded token path, raw-byte cap, token-content refusal, no-store response, and v1 capture marker.
- **Fixed**: the bounded Yahoo support capture now preserves Yahoo's first redirect response with manual redirect handling instead of converting it into an opaque transport failure. It still makes one fixed request and never forwards the bearer token to a redirect target.
- **Added**: a permanently available but tightly bounded, two-secret, rate-limited Yahoo support capture for an active account investigation. It accepts only a Clerk user id and numeric game key, follows the ordinary guarded Yahoo token-renewal path, then makes one fixed `users → game → leagues;out=teams` request. It returns the exact response bytes only to the authenticated operator with a no-store binary response, capture digest, byte count, upstream status, and correlation id; it never logs or returns the Yahoo URL, content type, provider error text, tokens, or parsed provider fields. Responses over 8 MiB and any body containing an access or refresh token held before or after renewal are refused.
- **Added**: an operator-only, two-secret, rate-limited Yahoo locator for the narrow case where a screenshot has an exact team-name SHA-256 digest and a numeric Yahoo game key but no league key. It makes exactly one fresh-token-only `users → game → leagues;out=teams` read, compares the raw UTF-8 provider names without normalization, and returns a full numeric league key only for one exact digest match; every other result is the closed `none`, `multiple`, or `unavailable` status. It never refreshes credentials, persists data, or returns names, GUIDs, team keys, URLs, tokens, raw provider data, or nonmatching league keys. A unique result remains a lead only: existing ownership verification and guarded recovery are still required.
- **Improved**: the existing two-secret Yahoo membership and exact-key recovery routes now optionally accept a lowercase SHA-256 digest of the exact UTF-8 team name, never the name itself. The verifier reports only a closed corroboration status. When supplied for recovery, the digest requires a fresh Yahoo login GUID and exactly one direct-league team with both that GUID and the exact provider-returned name bytes; it can disambiguate only multiple manager-GUID matches and still fails closed on incomplete identity data or any conflicting direct-owner/current-login marker. Digests, names, team keys, GUIDs, and provider bodies are never returned, logged, or stored.
- **Improved**: the read-only verifier now treats direct league metadata as usable only when its `league_key` exactly matches the requested league and every direct `team_key` is that league's canonical numeric team key. Mixed-type duplicate fields fail closed before parsing. This tightens existing verifier evidence only; it adds no Yahoo calls and does not change recovery.
- **Fixed**: recovery and membership parsers now count raw direct-field values before type filtering, so a valid key, name, or GUID paired with a malformed duplicate fails closed instead of being accepted. Recovery rejects malformed manager identities before ownership selection, while its established no-digest direct-owner path still permits genuinely absent or incomplete manager metadata. Valid Yahoo response shapes and recovery's exact raw-name digest semantics are unchanged.
- **Improved**: when the optional digest is supplied to the read-only verifier, its additional closed `teamNameDigestPresence` result counts exact raw-name matches across the validated direct league response without using manager GUIDs or authenticated identity. It makes no extra Yahoo calls and is unavailable for incomplete name data or an empty direct-team collection.
...
- **Fixed**: recovery now accepts Yahoo's sparse ownership shape when exactly one team is explicitly marked as owned and other teams omit the marker. Multiple positive markers and present-but-invalid markers still fail closed, while the existing fresh-login GUID fallback remains limited to responses with no positive marker and genuinely omitted ownership fields.
- **Fixed**: when the fresh authenticated Yahoo GUID matches multiple teams' manager lists, recovery now uses Yahoo's manager-level `is_current_login` marker only as a tie-breaker, and only if exactly one manager carries it, that exact manager's GUID equals the fresh authenticated GUID, the team does not have an explicit negative ownership marker, and every manager collection parsed completely. Invalid, conflicting, or multiple current-login markers fail closed; unique GUID matches keep their existing behavior.
- **Improved**: the read-only Yahoo membership verifier also probes Yahoo's direct logged-in-user teams collection and records only whether it parsed and whether the requested league produced zero, one, or multiple exact numeric team-key entries. Every counted row still requires a valid wrapper/entity and exactly one direct string `team_key`. A key containing the requested league prefix must end in a numeric team id or fails closed with a redacted malformed-target category; every other single string is ignored as unrelated without attempting to validate its Yahoo grammar. It keeps distinct closed failure categories for invalid wrappers, entity shapes, no direct `team_key` field, multiple direct `team_key` fields, and one non-string value. The extra evidence stays in redacted operator telemetry; it does not change the support response, recovery selection, or persistence behavior.
- **Improved**: failed recovery writes log one closed internal stage category so operators can distinguish credential, provider-shape, exact ownership-marker outcome, login/manager identity completeness and match cardinality, renew-chain, deadline, readback, and persistence failures without exposing the category or any customer/provider identifiers to the support client. Membership verification also logs its existing closed manager-comparison enum so the two identity sources can be reconciled without logging a GUID.
...
### v4 Widget URI with ESPN and Sleeper Credit Links (FLA-345)

- **Added**: a new v4 league widget (`ui://widget/user-session-v4.html`) whose footer links all three provider credits — Yahoo Fantasy to the official Yahoo Fantasy site, ESPN to the official ESPN Fantasy site, and Sleeper to the official Sleeper site. Its widget CSP `redirect_domains` allowlists `https://www.espn.com` and `https://sleeper.com` alongside the existing `https://flaim.app` and `https://sports.yahoo.com`, and each link uses the host's `openExternal` where available, falling back to the native anchor. The `get_user_session` descriptor now targets v4, and v4 joins the anonymous static-resource allowlist so widget templates stay readable without auth.
- **Unchanged**: published widget URIs are immutable ChatGPT cache keys, so v1, v2, and v3 keep serving their existing bodies with byte-identical read-result `_meta`; v3's body is not widened to the new links, because v3's published CSP does not allow those domains. The version-less HTTP fallback widget routes continue to serve the current (now v4) body.

### Draft, Keeper, and Matchup Mechanics Move Into Tool Descriptions (FLA-374)

- **Added**: `get_draft`'s description now states the per-provider reach of pick ownership — ESPN and Yahoo expose confirmed draft results but no current pick-ownership ledger, so their responses carry no `ownership` block at all, while only Sleeper reports current or future ownership and only supplies an exact board placement when it has enough draft-order evidence. It also names the three fields that were previously explained only in the shipped skill: current ownership reads solely from `ownership.picks[].currentOwnerTeamId`, `ownership.scope: changed_picks_only` is not a complete pick inventory, and `selectionInRound` (the round slot in a value such as 12.15) is not `draftColumn` (the stable board column).
- **Added**: the platform-dependent keeper field inventory now lives on the tools that return those fields. `get_league_info` names ESPN `keeperSettings`/`isKeeperLeague` and the raw per-team `keeperPlayerIds`/`futureKeeperPlayerIds`, Sleeper `leagueFormat` with the warning that `typeRaw` is an undocumented convention that must not be read on its own, and Yahoo `draftType`/`isAuctionDraft`/`canTradeDraftPicks`, present only when Yahoo's settings fetch succeeds, with no keeper-cost rule. `get_roster` names ESPN `keeperValue`/`keeperValueFuture` and the response-level `keeperValueUnit`, the historical-snapshot withholding of `keeperValueFuture`, and that Sleeper's resolved `keepers` passes through the current roster's list as Sleeper sends it. `get_roster`, `get_free_agents`, and `get_players` all note that Yahoo has only been observed returning `isKeeper.cost` as `false`.
- **Added**: `get_matchups` now states in its description and its `detail` parameter that opt-in player detail is ESPN football from `season_year` 2018 onward, and that anything else returns `MATCHUP_DETAIL_UNSUPPORTED`. That bound previously appeared only in the runtime error.
- **Added**: server instructions gained two cross-tool rules — keeper cost is a league-specific house rule Flaim never computes (only ESPN reports a keeper value, which it keeps with a player through a trade; Yahoo and Sleeper report none), so ask for the league's convention when no provider value settles it; and a missing league record never turns into asking the user for a numeric league ID or season value.
- **Changed**: the eight `season_year` parameter descriptions are now generated from one shared constant that also carries the per-sport convention (basketball and hockey use the cross-year start year, 2024 = 2024-25; football and baseball use the single season year, 2025 = 2025), so the convention cannot drift between tools.
- **Fixed (description audit)**: two independent audits caught follow-up inaccuracies in the text above and completed two clauses left unfinished by the earlier get_players ownership change (FLA-382/PR #290) — `get_players`' description and the server instructions now also cover the silent `SLEEPER_OWNERSHIP_UNAVAILABLE` path (league/roster/user load failure outside an active draft) alongside the existing draft-in-progress case, and give the causes of a null `league_status` as examples (unavailable credentials or rosters, a player result with no usable id, ambiguous Sleeper rostering) rather than one Sleeper-flavored explanation for all three platforms.
- **Unchanged**: tool names, titles, annotations, input and output schemas, and behavior. Description, parameter-description, and server-instruction text only, shipped under the continuous-review lane; portal scan confirmation is recorded in the PR.

### flaim-fantasy Skill Rewritten as the Analyst Playbook (FLA-345, FLA-406)

- **Changed**: `.agents/skills/flaim-fantasy/SKILL.md` was mostly a second copy of the tool reference. The tool `description:` strings and the server instructions now carry the mechanics they own (parameters, response fields, platform differences, wording prohibitions, error handling), and those surfaces are read live by every MCP client while the skill is a submission-time snapshot. The skill is now the judgment layer only: what Flaim is and cannot do, the tool-free setup and capability paths, the data-source rule, a compact context-gathering order, and a playbook per decision type (start/sit, waivers and pickups, trade evaluation, keepers and dynasty, matchup previews, draft picks, season history, multi-league comparisons), plus web-evidence, refusal, honesty, and response-style guidance.
- **Removed**: the per-tool reference section, the platform-details section, the season-year table, the error-type table, field and enum names, and the long tool-sequence example lists. Each of those facts is carried by a tool description or by the server instructions. Frontmatter is unchanged (`name`, `description`, `license: MIT`).
- **Added (FLA-406)**: a category-scoring judgment rule for matchup previews. In a category league the side total is the number of categories won rather than points, so the answer reasons category by category, and an empty category value, result, or side total is reported as not provided rather than as a zero.
- **Added**: waiver and pickup guidance now starts from cost, using the waiver priority and FAAB balance that `get_standings` reports where the platform provides them.
- **Added**: a web research section that makes current reporting and expert opinion required inputs to any current-decision advice. The skill ranks evidence as league data from Flaim, then current news, injury, stats, and team reporting from the web, then expert consensus, with the model's own judgment applied last to fit the league. It tells the model not to state a player's current team, role, or health from memory, to check the date on every source, to anchor on expert consensus and explain any departure, and to label advice as league-data-only when web research is unavailable. Each decision playbook names the research it needs. Setup and capability answers stay tool-free and research-free.
- **Changed**: `workers/fantasy-mcp/tests/integration/skill-contract.test.ts` now pins the safety-critical posture in the skill (provider-write boundary, tool-free paths, never asking for credentials, `flaim.app/leagues` routing, the data-source and no-inference rules, the category-scoring rule, the keeper-cost house rule), asserts that mechanics are *not* duplicated back into the skill, and asserts that every mechanic the rewrite removed is still present in `tools.ts` or `instructions.ts`.
- **Reviewed surface**: shipped skill text changes require a new OpenAI version and human review. This entry does not claim a submission was made.
...
- **Added**: A fifth route, `/internal/support/yahoo/verify-league-membership`, compares a reported full Yahoo league key against both the direct league teams collection and the signed-in user's teams for that game. It makes exactly two bounded read-only Yahoo GETs, treats direct ownership flags and matching manager GUIDs as affirmative evidence, and reports non-membership only when the direct response is structurally complete and either the two collections agree or its complete manager set excludes a fully validated live identity. League keys, Yahoo ids, GUIDs, names, provider bodies, and tokens never cross the response or audit-log boundary.
- **Fixed**: The membership verifier now emits one bounded structured log with a closed parse-stage reason for each Yahoo response. This distinguishes a valid empty collection from a missing or malformed nested entity without logging provider bodies, URLs, league keys, Yahoo ids, GUIDs, names, or tokens; endpoint output and ownership interpretation are unchanged.
- **Fixed**: The membership verifier preserves the authenticated Yahoo identity when a later teams collection is incomplete, and compares it only against a structurally complete manager list from every league team. A complete mismatch can now identify a different connected Yahoo account; partial manager data remains inconclusive. Live identity evidence takes precedence over an older stored GUID, with no additional Yahoo calls or persistence.
- **Fixed**: Nested Yahoo team metadata, native or counted manager collections, and manager entities serialized as a single object now count as complete membership evidence, matching provider shapes already handled by the Yahoo client. Flat team metadata and array-shaped manager entities remain supported, and every form still requires exactly one canonical nonblank GUID before a mismatch can be decisive. Ownership and manager evidence are read only from direct team metadata, never object-valued subresources.
