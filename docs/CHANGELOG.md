...
- **Added**: After a league-confirmed sync, the extension may show one account-scoped, neutral `Leave an honest review` invitation after the primary `Your Leagues` action. It never asks for a star value, offers a reward, gates the link by sentiment, or treats a click as a submitted review.
