...
- **Added**: Yahoo `get_roster` now requests `/players/stats;type=week` alongside the roster fetch — `;week=N` too, when the snapshot pins a specific week — and emits an additive `points` field per player (football only; Yahoo's `;week=` roster selector is itself football-only, and category/roto leagues carry no `player_points` resource even when the sport supports it). A genuine `0.00` total (e.g. a bye-week starter) is preserved as `0`; `points` is omitted entirely — never `null` — for a player with no usable stats sub-resource, matching how `isKeeper`/`team`/`status` are already omitted elsewhere in this handler. The coverage gate differs by snapshot type: on a `week` snapshot, a player's points are usable only when Yahoo's own echoed `player_points.coverage_type` is `week` AND `week` EXACTLY equals the requested week — any other week is treated as unusable for that player. On a `current` snapshot there is no requested week to check against, so instead every emitted player's points must share one single consistent week: the coverage week of the first player with usable (finite points, `type: 'week'`, positive-integer week) data becomes that response's week, and any other player whose echoed week differs is treated as unusable — so a season-to-date total, or players scored against different weeks, can never be mislabeled or mixed together as "this week's score". A response-level `pointsCoverage: { type: 'week', week: N }` is added whenever at least one player carried usable points under that rule, and `limitations.playerPointsAvailable: false` is added whenever the request was football but no player ended up with usable points. No output schema change: `GET_ROSTER_OUTPUT_SCHEMA.players` and `limitations` are both `looseObject()` passthrough, so this additive shape validates as-is (same precedent as `snapshot.leagueStatus`/FLA-284 above).
- **Note**: The `player_points` sub-resource's array index and coverage-echo behavior are inferred from Yahoo's own (sparse) documentation and fixture-tested only — there is no captured live `player_points` payload anywhere in this repo, and Yahoo cannot be exercised on the preview lane, so live confirmation happens against production after merge. Sub-resource reads (`selected_position`, now also `player_points`) scan the player array by key instead of assuming a fixed index, since Yahoo's ordering isn't documented or guaranteed.
