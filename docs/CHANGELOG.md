...
- **Fixed**: ESPN `get_transactions` once again serves its structured `mTransactions2` source as primary. The transport now sends only the minimal `filterType` fantasy-filter: ESPN returns HTTP 400 for any filter carrying `limit`/`offset` (with or without sort fields), which was the entire cause of the previous 100% structured-path failure rate. The view exposes no pagination; each pinned scoring period returns ESPN's own full result set for that period, fetched once.
