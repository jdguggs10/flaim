...
- **Changed**: Yahoo refresh-token grants now include the registered `redirect_uri` again, matching Yahoo's documented refresh-token examples after production logs showed omission still returning Yahoo `429` / `Too many failed requests`.
- **Changed**: Yahoo access tokens are now refreshed only at near-expiry instead of using the previous 5-minute proactive buffer, after production logs showed early refresh attempts with about 90 seconds remaining returning Yahoo `429` / `Too many failed requests`.
...
- **Changed**: Yahoo reconnect again validates the returned refresh token before saving credentials, so a malformed refresh setup fails during reconnect instead of appearing healthy until the first 1-hour access-token expiry.
