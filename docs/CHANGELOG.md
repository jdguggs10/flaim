...
- **Added**: The August football kickoff broadcast is now a repo-owned React Email template, matching the sent update's copy, 36px Flaim mark, card-aligned header inset, attribution-tagged Flaim links, three-action layout, and Resend unsubscribe placeholder. `email:export` now also produces a plain-text fallback beside each broadcast HTML export.
