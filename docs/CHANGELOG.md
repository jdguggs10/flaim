...
- **Added**: A fifth route, `/internal/support/yahoo/verify-league-membership`, compares a reported full Yahoo league key against both the direct league teams collection and the signed-in user's teams for that game. It makes exactly two bounded read-only Yahoo GETs, treats direct ownership flags and matching manager GUIDs only as affirmative evidence, and reports non-membership only when both structurally valid responses explicitly agree. League keys, Yahoo ids, GUIDs, names, provider bodies, and tokens never cross the response or audit-log boundary.
