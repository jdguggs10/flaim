...
- Public chat warm context: the web app still caches Gerry session context in Supabase (`chat_context_cache`) as groundwork from the original live-turn public demo. The homepage preset flow now reads cache-backed answers from `demo_answer_cache`, so visitors can read recently refreshed outputs without triggering live provider inference on every click.
