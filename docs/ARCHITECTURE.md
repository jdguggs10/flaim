...
- Public demo refresh pipeline: an external private runner uses Gemini CLI in headless mode with static MCP bearer auth to populate `public_demo_answer_cache` out of band on a scheduled cadence. The website only reads cached answers — it never triggers refresh runs. The shared contract between the website and the runner is the cache key format: `{presetId}:{sport}:{promptVersion}:{contextVersion}`, where version constants are defined in `web/lib/public-chat.ts`.
