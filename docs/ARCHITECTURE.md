...
For web and extension refreshes, current ESPN leagues are discovered inside the request and historical seasons are processed by a Cloudflare Workflow. The workflow owns the ESPN sync lease after the request returns, checkpoints each league-season through a fenced Supabase RPC, and can resume after worker retries. A versioned full-repair marker makes the first scan exhaustive; later scans skip existing historical rows. `get_ancient_history` remains a fast read-only index over the rows already committed by that workflow. MCP-triggered refresh remains synchronous until the separately gated MCP behavior change ships.
