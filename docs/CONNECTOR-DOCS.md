...
`get_free_agents` note: every response carries a normalized envelope (`leagueId`, `seasonYear`, `position`, `count`, `ordering`, `capabilities`, `ownershipScope`) plus normalized per-player fields where derivable (`acquisitionState` and `waiverClearsAt` on ESPN only, `id`, `team`); prefer these over the legacy provider fields, which otherwise remain present. ESPN and Yahoo include platform-wide ownership percentages and sort by ownership. Sleeper returns available-player identities without ownership percentages. ESPN entries in all four sports also carry `seasonPoints`, `pointsPerGame`, and `projectedSeasonPoints` for the season split, each `null` when the league does not score in points (category and rotisserie leagues). ESPN football entries no longer carry the raw `stats` map; ESPN baseball, basketball, and hockey entries still do. A response that would exceed the serialized tool-result size limit drops trailing entries, sets `count` to what was returned, and adds `truncated: true`; entries are dropped from the end of the response's `ordering` (lowest platform-wide rostered rate on ESPN and Yahoo, last alphabetically on Sleeper).
`get_players` note: ESPN and Yahoo may return league ownership fields (`league_status`, `league_team_name`, `league_owner_name`) when available. Sleeper always evaluates league ownership against the selected league's current rosters, adding a Sleeper-only `league_team_id`, except during an active draft, when it returns an error instead of an answer; Sleeper's market/global ownership stays unavailable. Trust a non-null `league_status` directly — verify with `get_league_info` plus `get_roster` only when `league_status` itself is absent or null.
