...
`get_players` note: ESPN and Yahoo may return league ownership fields (`league_status`, `league_team_name`, `league_owner_name`) when available. Sleeper market-percentage ownership remains unavailable, but selected-league availability is independently verified by checking each matched player ID against every current roster for the exact league. Sleeper returns `availability_status` (`ROSTERED` or `AVAILABLE`) plus roster/team/owner fields when rostered. If league ownership fields are absent or null, verify with `get_league_info` plus `get_roster`.
